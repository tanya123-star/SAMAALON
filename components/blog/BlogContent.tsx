...
function highlight(text: string, q: string) {
  ...
  return (
    <>
      {before}
      <span className="font-bold text-[#1C2A28] bg-amber-100/60">{match}</span>
      {after}
    </>
  );
}
...
export function BlogContent({
  ...
  const suggestions = useMemo(() => {
    ...
    if (!q) return [] as Post[];
    const pool = selectedCategory === "all" ? posts : posts.filter((p) => p.category?.slug === selectedCategory);
    ...
    const scored = matched
      .map((p) => {
        const t = p.title.toLowerCase().includes(q) ? 0 : p.category?.name.toLowerCase().includes(q) ? 1 : 2;
        return { p, t };
      })
      .sort((a, b) => a.t - b.t || (b.p.publishedAt ? new Date(b.p.publishedAt).getTime() : 0) - (a.p.publishedAt ? new Date(a.p.publishedAt).getTime() : 0))
      .slice(0, 5)
      .map((x) => x.p);
    return scored;
  ...
  function buildUrl(next: { q?: string; category?: string }) {
    ...
    const catVal = next.category !== undefined ? next.category : selectedCategory;
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-8 flex flex-col gap-3">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <div ref={wrapperRef} className="relative ml-auto w-full min-w-[220px] max-w-[380px] mt-3 lg:mt-0">
              ...
              <div className="relative flex items-center">
                ...
                <input
                  ...
                  onKeyDown={onKeyDown}
                  placeholder="Search"
                  className="w-full rounded-lg border border-[#1C2A28]/10 bg-[#FAF8F5] py-3 pl-9 pr-8 text-sm text-[#1C2A28] placeholder:text-[#5A6B68]/60 focus-visible:border-[#2D6A4F] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#2D6A4F]/20"
                  autoComplete="off"
                />
                {query ? (
                  <button type="button" onClick={handleClear} aria-label="Clear search" className="absolute right-2 rounded-md p-1 text-[#5A6B68] hover:bg-white hover:text-[#1C2A28]">
              ...
              {open && query.trim().length >= 1 ? (
                <div id="blog-search-listbox" role="listbox" className="absolute left-0 right-0 top-full z-20 mt-2 max-h-80 overflow-auto rounded-lg border bg-white shadow-md">
                  {suggestions.length === 0 ? (
                    <div className="px-3 py-4 text-xs text-[#5A6B68]">
                      <p className="font-semibold text-[#1C2A28]">No travel stories found.</p>
                    ...
                    suggestions.map((s, i) => (
                      ...
                      >
                        <img
                          src={s.featuredImage || "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?auto=format&fit=crop&w=200&q=80"}
                          alt=""
                          className="h-14 w-14 shrink-0 rounded-lg object-cover bg-slate-100"
                          onError={(e) => ((e.currentTarget as HTMLImageElement).style.display = "none")}
                        />
                        <span className="min-w-0 flex-1">
                          <span className="block truncate text-xs font-bold text-[#1C2A28]">{highlight(s.title, query.trim())}</span>
                          <span className="block text-[10px] font-semibold uppercase tracking-wider text-[#5A6B68]">{s.category?.name ?? "Travel"}</span>
                        </span>
                      </button>
                    ))
                  )}
          ...
          {(selectedCategory !== "all" || query.trim() || qParam) && (
            <p className="mt-4 text-xs text-[#5A6B68]">
              {query.trim() || qParam ? (
                <>
                  Search results for &ldquo;<span className="font-bold text-[#2D6A4F]">{query.trim() || qParam}</span>&rdquo; ·{" "}
                </>
              ) : (
                <>Showing: </>
              )}
              {selectedCategory !== "all" ? (
                <span className="font-bold text-[#2D6A4F]">{categories.find((c) => c.slug === selectedCategory)?.name}</span>
              ) : query.trim() || qParam ? (
                <span>
      ...
      <section className="mx-auto w-full max-w-7xl px-4 sm:px-8 py-16">
        {filteredPosts.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-[#1C2A28]/20 bg-white p-12 text-center">
            ...
            <p className="mt-1 text-xs text-[#5A6B68]">Try searching for beaches, resorts, travel guides, or things to do.</p>
          </div>
        ) : (
