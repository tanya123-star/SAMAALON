...
async function updatePost(formData: FormData) {
  ...
  // --- new remote URL ---
  const newRemoteUrl = String(formData.get("newRemoteUrl") ?? "").trim() || undefined;
    ...
    <div>
      ...
      <form action={updatePost} className="mt-4 grid gap-2 rounded-lg border p-4">
        ...
        <input
          ...
          className="rounded border px-2 py-1 text-sm w-full mt-2 hidden"
        />
        {/* File upload: hidden input + button */}
        <div className="mt-3">
          <h3 className="text-xs font-medium mb-1">Or upload new file</h3>
          <input
          ...
          <button
            type="button"
            onClick={() => (document.querySelector('input[name="newUploadedFile"]') as HTMLInputElement).click()}
            className="rounded border px-3 py-2 text-xs hover:bg-muted transition-colors"
          >
            Browse File
          </button>
