...
export default async function EditRoomTypePage({ params }: { params: Promise<{ id: string; roomTypeId: string }> }) {
  ...
  return (
    <div>
      <Link href={`/admin/accommodations/${accommodationId}/room-types`} className="text-sm text-muted-foreground hover:text-foreground">
        ← Back to Room Types
      </Link>
      <h1 className="mt-2 text-xl font-bold">Edit Room Type: {roomType.name} — {acc.name}</h1>
      <EditRoomTypeForm roomType={roomTypeData} updateRoomType={updateRoomType} />
    </div>
  );
}
