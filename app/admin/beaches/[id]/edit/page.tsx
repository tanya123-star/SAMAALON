...
async function updateBeach(formData: FormData) {
  ...
  const existingFiltered = keptIds.length
    ? (existing.images as unknown as { id: string; url: string }[]).filter(
  ...
  const effectiveExisting = hasKeptInput
    ? existingFiltered
    : (existing.images as unknown as { id: string; url: string }[])
  ...
  for (const f of uploadedFiles) {
    if (!f || typeof (f as File).name !== "string" || (f as File).size === 0)
      continue
    try {
...
export default async function EditBeachPage({
  params,
}: {
  ...
  const initialImages = (
    (beach.images as unknown as { id: string; url: string }[]) || []
  ).map((img) => ({
    id: img.id,
    url: img.url,
