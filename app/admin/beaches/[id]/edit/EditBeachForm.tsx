...
function RemotePreview({ url }: { url: string }) {
  ...
  try {
    ...
    if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
      urlError = `Invalid URL protocol — ${url}`
    } else {
      safeUrl = parsed.toString()
    }
  } catch {
    urlError = `Invalid URL format — ${url}`
  }
  if (urlError) {
    return <p className="text-xs text-red-600">{urlError}</p>
  }
...
export function EditBeachForm({ beach, initialImages, updateBeach }: Props) {
  ...
  return (
    ...
    >
      ...
      <textarea
        name="description"
        defaultValue={beach.description}
        required
        className="rounded border px-2 py-1 text-sm"
        rows={2}
      />
      ...
      <div className="grid gap-2 sm:grid-cols-4">
        <input
          name="entranceFee"
          defaultValue={beach.entranceFee ?? ""}
        ...
        <input
          name="openingHours"
          defaultValue={beach.openingHours ?? ""}
          placeholder="Opening Hours"
          className="rounded border px-2 py-1 text-sm"
        />
        <input
          name="contactInfo"
          defaultValue={beach.contactInfo ?? ""}
          placeholder="Contact Info"
          className="rounded border px-2 py-1 text-sm"
        />
        <input
          name="googleMapsUrl"
          defaultValue={beach.googleMapsUrl ?? ""}
