  ...
  try {
    if (entity === "beaches") {
      ...
      for (const r of rows) {
        ...
        if (r.location.toLowerCase().includes(q.toLowerCase()))
          set.add(r.location)
      ...
      for (const r of rows as unknown as {
        ...
        if (r.name.toLowerCase().includes(q.toLowerCase())) set.add(r.name)
        if (r.slug.toLowerCase().includes(q.toLowerCase())) set.add(r.slug)
        if (r.beach?.name.toLowerCase().includes(q.toLowerCase()))
          set.add(r.beach.name)
        if (set.size >= take) break
      }
      if (set.size === 0)
        (rows as unknown as { name: string }[]).forEach((r) => set.add(r.name))
      suggestions = Array.from(set).slice(0, take)
    } else if (entity === "blog") {
      ...
      for (const r of rows as unknown as {
        ...
        if (r.slug.toLowerCase().includes(q.toLowerCase())) set.add(r.slug)
        if (r.category?.name.toLowerCase().includes(q.toLowerCase()))
          set.add(r.category.name)
        if (set.size >= take) break
      }
      if (set.size === 0)
        (rows as unknown as { title: string }[]).forEach((r) =>
          set.add(r.title)
        )
      suggestions = Array.from(set).slice(0, take)
      ...
      for (const r of rows as unknown as {
        ...
        if (r.user?.name?.toLowerCase().includes(q.toLowerCase()))
          set.add(r.user.name)
        else if (r.user?.email.toLowerCase().includes(q.toLowerCase()))
          set.add(r.user.email)
        if (r.beach?.name.toLowerCase().includes(q.toLowerCase()))
          set.add(r.beach.name)
        if (r.accommodation?.name.toLowerCase().includes(q.toLowerCase()))
          set.add(r.accommodation.name)
