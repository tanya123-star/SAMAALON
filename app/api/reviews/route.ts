...
export async function POST(req: NextRequest) {
  ...
  const review = await prisma.$transaction(async (tx) => {
    const r = await tx.review.create({ data: { rating, comment, userId, beachId, accommodationId } as never });
    if (beachId) {
      const agg = await tx.review.aggregate({ where: { beachId }, _avg: { rating: true }, _count: { rating: true } });
      await tx.beach.update({ where: { id: beachId }, data: { avgRating: agg._avg.rating ?? undefined, reviewCount: agg._count.rating } });
    }
    if (accommodationId) {
      const agg = await tx.review.aggregate({ where: { accommodationId }, _avg: { rating: true }, _count: { rating: true } });
      await tx.accommodation.update({ where: { id: accommodationId }, data: { avgRating: agg._avg.rating ?? undefined, reviewCount: agg._count.rating } });
    }
    return r;
  });
